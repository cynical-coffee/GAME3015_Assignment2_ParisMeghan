...
bool Game::Initialize()
{
	...
	mCamera.SetPosition(0, 2, 3.5f);
	mCamera.Pitch(20 * XM_PI / 360);
	mCamera.RotateY(360 * XM_PI / 360);
...
void Game::LoadTextures()
{
	...
	//Ocean
	auto OceanTex = std::make_unique<Texture>();
	OceanTex->Name = "OceanTex";
	OceanTex->Filename = L"Textures/Ocean.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), OceanTex->Filename.c_str(),
		OceanTex->Resource, OceanTex->UploadHeap));

	mTextures[OceanTex->Name] = std::move(OceanTex);

	//Cloud
	auto CloudTex = std::make_unique<Texture>();
	CloudTex->Name = "CloudTex";
	CloudTex->Filename = L"Textures/Ocean.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
...
void Game::BuildDescriptorHeaps()
{
	...
	auto OceanTex = mTextures["OceanTex"]->Resource;
	...
	//Ocean Descriptor
	hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	srvDesc.Format = OceanTex->GetDesc().Format;
	md3dDevice->CreateShaderResourceView(OceanTex.Get(), &srvDesc, hDescriptor);

	//Cloud Descriptor
	hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	srvDesc.Format = CloudTex->GetDesc().Format;
	md3dDevice->CreateShaderResourceView(CloudTex.Get(), &srvDesc, hDescriptor);
...
void Game::BuildMaterials()
{
	...
	Ocean->Name = "Ocean";
	Ocean->MatCBIndex = 3;
	Ocean->DiffuseSrvHeapIndex = 3;
	Ocean->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	Ocean->FresnelR0 = XMFLOAT3(0.05f, 0.05f, 0.05f);
	Ocean->Roughness = 1.0f;

	mMaterials["Ocean"] = std::move(Ocean);

	auto Cloud = std::make_unique<Material>();
	Cloud->Name = "Cloud";
	Cloud->MatCBIndex = 4;
	Cloud->DiffuseSrvHeapIndex = 4;
	Cloud->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	Cloud->FresnelR0 = XMFLOAT3(0.05f, 0.05f, 0.05f);
	Cloud->Roughness = 1.0f;
